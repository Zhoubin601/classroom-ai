      ...
      >
        ...
        <div class="relative mb-2">
          <div
            ...
            :class="s.present ? 'border-indigo-400' : 'border-slate-200'"
          >
            <img
              v-if="s.avatarUrl"
              :src="s.avatarUrl"
          ...
          <span
            ...
            :class="[
              ...
              s.poseState === 'UP' ? 'bg-emerald-500' : 'bg-rose-500'
            ]"
          ></span>
        </div>
...
const activeFilter = ref<'all' | 'present' | 'up' | 'down' | 'absent'>('all')

const filterTabs = [
  { key: 'all', label: '全部学生' },
  { key: 'present', label: '实到在场' },
  { key: 'up', label: '抬头专注' },
  { key: 'down', label: '低头预警' },
...
const getFilterCount = (key: typeof activeFilter.value) => {
  switch (key) {
    case 'present':
      return props.students.filter(s => s.present).length
    case 'up':
      return props.students.filter(s => s.poseState === 'UP').length
    case 'down':
      return props.students.filter(s => s.poseState === 'DOWN').length
    case 'absent':
      return props.students.filter(s => !s.present || s.poseState === 'ABSENT').length
    case 'all':
    default:
...
const filteredStudents = computed(() => {
  switch (activeFilter.value) {
    case 'present':
      return props.students.filter(s => s.present)
    case 'up':
      return props.students.filter(s => s.poseState === 'UP')
    case 'down':
      return props.students.filter(s => s.poseState === 'DOWN')
    case 'absent':
      return props.students.filter(s => !s.present || s.poseState === 'ABSENT')
    case 'all':
    default:
