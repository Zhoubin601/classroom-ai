...
class CourseAcceptanceRegressionTest {
    interface Checked { void run() throws Exception; }
    private final java.util.List<org.junit.jupiter.api.DynamicTest> cases = new java.util.ArrayList<>();
    static void check(boolean ok, String detail) { org.junit.jupiter.api.Assertions.assertTrue(ok, detail); }
    void probe(String name, Checked test) {
        cases.add(org.junit.jupiter.api.DynamicTest.dynamicTest(name, () -> {
            try { test.run(); }
    ...
    static class Imports {
        ...
        Imports() {
            Major major = Major.builder().id(1L).department("Department A").majorCode("SE").build();
    ...
    static class Content {
        ...
        Content() {
            ...
            when(offerings.findByCourseId(1L)).thenReturn(List.of(CourseOffering.builder().id(10L).course(course).teacherCode("REVIEW-T").build()));
            when(teachers.findByOfferingId(10L)).thenReturn(List.of(CourseOfferingTeacher.builder().teacherId(1L).teacherCode("REVIEW-T").build()));
            when(revisions.saveAndFlush(any())).thenAnswer(i -> i.getArgument(0));
        }
        ContentRevisionDTO dto(Integer lock) {
            return ContentRevisionDTO.builder().draftId(10L).publishVersion(0).description("STALE text").assessmentMethod("exam")
                .objectives("learn").lockVersion(lock).build();
        }
    }
    @org.junit.jupiter.api.TestFactory
    java.util.List<org.junit.jupiter.api.DynamicTest> acceptanceRegressions() {
        ...
        probe("09 lock reset across drafts must not accept old page", () -> {
            Content f = new Content();
            when(f.revisions.findFirstByCourseIdAndStatusOrderByIdDesc(1L, "DRAFT"))
        ...
        probe("13 archive editing preserves unknown historical creator", () -> {
            Imports f = new Imports();
            CourseServiceImpl service = new CourseServiceImpl(f.courses, mock(CourseOfferingRepository.class),
                mock(com.classroom.ai.repository.StudentRepository.class), mock(OfferingStudentEnrollmentRepository.class), f.majors);
            Course historical = Course.builder().id(1L).courseCode("R1").department("Department A")
                .description("Official content").build();
            when(f.courses.findForUpdate(1L)).thenReturn(Optional.of(historical));
            when(f.courses.findByCourseCode("R1")).thenReturn(Optional.of(historical));
            when(f.courses.save(any())).thenAnswer(i -> i.getArgument(0));
            CourseDTO dto = CourseDTO.builder().id(1L).courseCode("R1").courseName("Renamed")
                .department("Department A").majorCode("SE").credits(3.0).hours(48).courseType("core").build();
            Course saved = service.saveCourse(dto);
            check(saved.getCreatedBy() == null, "unknown historical creator must remain unknown");
            check("review_director".equals(saved.getUpdatedBy()), "current editor must be recorded");
            check("Official content".equals(saved.getDescription()), "archive edits must preserve published content");
        });
        return cases;
    }
}
